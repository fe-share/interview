# interview

收集和整理的前端面试问题及资料


## HTML

1. [HTML5新标签有哪些](./html/tag.md)
2. [什么是 web 语义化,有什么好处](./html/semantization.md)
3. [前端需要注意哪些SEO?](./html/seo.md)


## CSS

1. [link与@import的区别](./css/link&import.md)
2. [px、em、rem、vw、vh？rem 的根节点样式在什么时候设置？](./css/units.md)
3. [响应式布局的原理](./css/responsive.md)
4. [盒模型有哪几种？怪异模式和标准模式？](./css/box.md)
5. [float清除浮动](./css/float.md)
6. [BFC 块级上下文、IFC，实现双栏高度对齐](./css/bfc.md)
7. [sprite（雪碧图）知道吗？svg 雪碧图了解吗？](./css/sprite.md)
8. [伪类是什么？有哪些？会有哪些兼容性问题？如何处理？](./css/pseudo-class.md)


## JavaScript

1. [typeof](./js/type.md)
2. [弱类型转换规则](./js/transform.md)
3. [原型链](./js/prototype.md)
4. [继承](./js/extend.md)
5. [实现call, apply, bind](./js/call.md)
6. [垃圾回收机制](./js/trash.md)
## HTTP
1. [http code](./http/code.md)
2. [http 缓存逻辑是怎样的？协商缓存与强缓存？Last-Modified / Etag / Expires / Cache-Control ?](./http/cache.md)
3. [http1.0和http1.1的区别](./http/protocol.md)
4. [跨域是什么？http协议中如何判断跨域？如何解决跨域问题？](./http/cross-origin.md)
## React
1. [ReactDOM.render](./react/reactDOM/render.md)
## Babel
1. [babel是如何工作的](./babel/babel.md)
## 收集的一些比较有意思的面试题
1. [css实现一个九宫格](./html/grid.html)